<?php
    global $config, $lazyload_status, $loader, $registry, $db, $session;
...
/*VARIABLES*/	
	...
	$new_text =  (isset($soconfig_pages[$lang]["new_text"]) ? $soconfig_pages[$lang]["new_text"] : 'NEW');
	$sale_text = (isset($soconfig_pages[$lang]["sale_text"]) ? $soconfig_pages[$lang]["sale_text"] : 'SALE');
	$quick_view_text = (isset($soconfig_pages[$lang]["quick_view_text"]) ? $soconfig_pages[$lang]["quick_view_text"] : 'QuickView') ;
				...
				<div class="item-inner product-thumb transition">
				 <div class="product-image-container <?php if(isset($soconfig_pages["secondimg"]) && $soconfig_pages["secondimg"] =='2' && isset($product['thumb2'])){ echo "second_img";} ?>">
				    <?php if($product_image){ ?>
						  <div class="image">
							 <a class="rspl-image <?php echo $lazyload_text?>" 
							    href="<?php echo $product['href'] ?>" target="<?php echo $item_link_target ?>"
							    title="<?php echo $product['name'] ?>">
								 <img  data-src="<?php echo $product['thumb'];?>" src="<?php echo $thumb; ?>" alt="<?php echo $product['name'] ?>">
					...
					if ($product_info['date_available'] >= $day_number_to_range) :
						?>
						<span class="label label-new"><?php echo $new_text ; ?></span>
						<?php endif; ?>
					<?php endif; ?>
					...
					<?php if (!isset($quick_status) || $quick_status != 0) : ?>
					 <!--full quick view block-->
						...
						<a class="quickview iframe-link hidden-md hidden-sm hidden-xs" data-fancybox-type="iframe"  href="<?php echo $our_url->link('product/quickview','product_id='.$product['product_id']);?>">  <?php echo  $quick_view_text; ?></a>
